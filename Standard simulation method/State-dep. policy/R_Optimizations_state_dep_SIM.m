...
M_num=5;%the number of machines in the production line
...
%     filename = ['OPT_BL2_', num2str(i), '.mat'];
...
for x2=1:size(OM,2)
    ...
    for x1=1:size(C_H,2)
        ...
        for rep=1:REP
            ...
            filename=['OPT_UBL5M1-3_', num2str(rep), '.mat'];
            load(filename);
            ...
            fun=@objective_function_state_dep_SIM;%the objective function
            ...
            [~,AVG_SynNUM]=objective_function_state_dep_SIM(sol);
            Syn_Policy(rep,:)=sol;
            min_Cost(rep,:)=fval;
        ...
        %% If the production line changes, the corresponding filename ("UBL5M1-3") should be modified.
        %%%%%%%%%%%%%%%%%%%% the parameters to be changed: the folder name related to ①-3 the production line: REAL,BL,UBL1,UBL2 %%%%%%%%%%%%%%%%
        writematrix(min_Cost,".\dep_results_UBL5M1-3_SIM\"+str+".xls",'Sheet',1,'Range','A2');
        writematrix(AVG_No_Syn,".\dep_results_UBL5M1-3_SIM\"+str+".xls",'Sheet',2,'Range','A2');
        writematrix(Syn_Policy,".\dep_results_UBL5M1-3_SIM\"+str+".xls",'Sheet',2,'Range','C2');
        writematrix(elapsed_time,".\dep_results_UBL5M1-3_SIM\"+str+".xls",'Sheet',3,'Range','A2');
    end
end
