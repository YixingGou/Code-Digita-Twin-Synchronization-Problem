...
%         j
        for n=1:N
            Y0_BN(j)=y_observed(1,(j-1)*Delta+2);
            Y_BN(j,n)=y_observed(1,(j+n-1)*Delta+2);%the state of bottleneck at time n*Delta in the jth combination
            Y0_LM(j)=y_observed(2,(j-1)*Delta+2);
            Y_LM(j,n)=y_observed(2,(j+n-1)*Delta+2);%the state of last machine at time n*Delta in the jth combination
%             h(j,n)=H(j+n-1);%the synchronization action at each period in the jth combination
            ...
            if h(j,n)==1
                ...
                if isempty(k)==1%if k is empty, it means that there is no synchronization during the interval[Delta,n*Delta).The predicted performance is estimated based on the initial state.
                    ...
                    E_N_predict(j,n)=sum(e_N)/size_x1;%the predicted performance is estimated with the average of the number of parts within the periods that the states are equal to 1 at the beginning of these periods 
                    e_N=zeros();
