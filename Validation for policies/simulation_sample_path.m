...
E=r./(p+r);%machine effciency
[~, BN] = min(E);%find the location of the bottleneck
% Fs_No=zeros(REP_No,M_num);%to record the probability of starvation for each machine over Exp_No simulation replications
TH_No=zeros(REP_No,M_num);%to record the throughput of each machine during time interval [t1,t2) over Exp_No simulation replications
TH_M=zeros(REP_No,N*Delta+1);%to record the throughput of last machine during the whole simulation period over Exp_No simulation replications
for k=1:REP_No
    ...
    M_state(2,:)=M_state(1,:);%the 2nd line in array M_state is the states at time 1, which is setted to be the same as the inital states
    WIP=zeros(N*Delta+1,M_num-1);%to record the buffer level of each buffer at each time
    WIP(1,:)=WIP_0;%the 1st row in array WIP is used to record the inital buffer levels
    TH=zeros(N*Delta+1,M_num);%to record the number of parts produced by each machine at each time under one simulation replication
    Fs=zeros(N*Delta+1,M_num-1);%to record the number of stravation
    %%%%%%%%%%%%%%%%%%
    ...
    throughput=sum(TH(t1+2:t2+1,:));%the total number of parts produced by each machine during [t1,t2) under one simulation replication
    TH_No(k,:)=throughput;
    TH_M(k,:)=TH(:,M_num)';%the total number of parts produced by the last machine during the whole simulation period ( REP_No *(N*Delta+1) )
    % E_No(k,:)=throughput/(t2-t1);
    % Fs_No(k,:)=sum(Fs(t1+2:t2+1,:))/(t2-t1); 
end
TH_No_basic=sum(TH_No,1)/REP_No;%the average number of parts produced by each machine during [t1,t2) over REP_No simulation replications
