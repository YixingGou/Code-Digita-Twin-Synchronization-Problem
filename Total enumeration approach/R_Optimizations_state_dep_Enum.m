...
%     filename = ['OPT_REAL_', num2str(i), '.mat'];
...
%     C_H=[1,10,20,30,40,50];
%     elapsed_time=zeros(size(C_H,2),1);   
    C_H=1;
    for x1=1:size(C_H,2)
...
%         str = ['state_dep_M', num2str(Position_machine), '_c_H=',num2str(c_H)];
        str = ['enum_state_dep_M', num2str(Position_machine), '_c_H=',num2str(c_H)];
    ...
    %         %% the machine being observed: Middle(ceil(M_num/2)) or Last(M_num)
    %         %%%%%%%%%%%%%%%%%%%% the parameters to be changed: ② the observed machine %%%%%%%%%%%%%%%%
    %         OM=M_num/2;%OM /in {M_num/2, M_num}
    %         Position_machine=ceil(OM);%the position of the observed machine
            ...
            b=0;
            for l=1:L
                y1(l)=y_observed((l-1)*Delta+2);%the state of the observed machine at period l(l=0,1,…,L-1)
                if y1(l)==1
                    a=a+1;
...
%             AVG_No_Syn(rep,:)=AVG_SynNUM;
            %% optimization: enumerating all the possible decisions%%%
            h1=zeros(2^N,N);
            h0=zeros(2^N,N);
            AVG_Cost=zeros(2^N,2^N);
            AVG_SynNUM=zeros(2^N,2^N);
            for i=1:2^N
                i
                for n1=1:N
                    b1=dec2bin(i-1,N);
            %     b2=dec2bin(i-1,N);
            %     Y(j,n)=str2double(b(n));
                    h1(i,n1)=str2double(b1(n1));
                end
                for j=1:2^N
            %         j
            %         h0(i,n)=str2double(b2(n));
                    for n2=1:N
                         b2=dec2bin(j-1,N);
                         h0(j,n2)=str2double(b2(n2));
                    end
            %     end
                    H=[h1(i,:),h0(j,:)];
                    [AVG_cost,AVG_synNUM]=objective_function_state_dep_SP(H);
                    AVG_Cost(i,j)=AVG_cost;
                    AVG_SynNUM(i,j)=AVG_synNUM;
                end
            end
            [min_Cost(rep),min_index]=min(AVG_Cost(:));
            [row, col] = ind2sub(size(AVG_Cost), min_index);
            H1=h1(row,:);
            H0=h0(col,:);
            Syn_Policy(rep,:)=[H0,H1];
            AVG_No_Syn(rep)=AVG_SynNUM(row,col);

            save_file = ['intermediateResult_', str, '.mat'];
            if mod(rep, 1) == 0
                save(save_file, 'min_Cost', 'AVG_No_Syn', 'Syn_Policy');
%                 fprintf('Iteration %d: Data saved to %s\n', w, save_file);
            end
        end
%         elapsed_time = toc;
        elapsed_time = toc(iterationStartTime);% Record the elapsed time of the current iteration
        %% the filename is'state_dep_M6_c_H=50.xls' when c_H=50 and observing M6(middle machine).If c_H and M are changed, the corresponding filename should be modified.
        %%%%%%%%%%%%%%%%%%%% the parameters to be changed: the folder name related to ①-3 the production line:BL,UBL1,UBL2 %%%%%%%%%%%%%%%%
%         writematrix(min_Cost,".\dep_results_BL1_SP\Middle\"+str+".xls",'Sheet',1,'Range','A2');
%         writematrix(AVG_No_Syn,".\dep_results_BL1_SP\Middle\"+str+".xls",'Sheet',2,'Range','A2');
%         writematrix(Syn_Policy,".\dep_results_BL1_SP\Middle\"+str+".xls",'Sheet',2,'Range','C2');
%         writematrix(elapsed_time,".\dep_results_BL1_SP\Middle\"+str+".xls",'Sheet',3,'Range','A2');
